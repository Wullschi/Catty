...
final class AudioEngineTestsli: XMLAbstractTest {
    ...
    func testHash() {
        ...
        let audioFileURL1 = testBundle.url(forResource: "guitar_chords", withExtension: "mp3")
        let audioFileURL2 = testBundle.url(forResource: "bling_short", withExtension: "mp3")
        do {
...
//            let playerli = try? AKAudioPlayer(file: readTape)
//            playerli?.connect(to: mix)
//            try? AudioKit.start()
//            playerli?.play()
            let player2 = try? AVAudioPlayer(contentsOf: readTape.url)
            player2?.play()
...
//            let program = self.getProgramForXML(xmlFile: "speakandwaittest")
//            let scene = self.createScene(program: program, soundEngine: soundEngine)
//            try recorder.record()
//            scene.startProgram()
...
//            let tape = try AKAudioFile()
//            let recorder = soundEngine.addNodeRecorderAtMainOut(tape: tape)
//            let program = self.getProgramForXML(xmlFile: "StartSoundTest")
//            let scene = self.createScene(program: program, soundEngine: soundEngine)
//            try recorder.record()
//
//            scene.startProgram()
...
//
//
//    private func createScene(program: Program, soundEngine: AudioEngine) -> CBScene {
//        let sceneBuilder = SceneBuilder(program: program).withFormulaManager(formulaManager: FormulaManager()).withSoundEngine(soundEngine: soundEngine)
