...
@objc open class ProjectService: NSObject {
    ...
    @objc static func copyProject(sourceProjectName: String, sourceProjectID: String, destinationProjectName: String) {
        ...
        let uniqueDestinationProjectName = Util.uniqueName(destinationProjectName, existingNames: ProjectService.getAllProjectNames())

        let destinationProjectPath = ProjectService.getProjectPath(projectName: uniqueDestinationProjectName!, projectID: nil)

        let fileManager = CBFileManager.shared()
        fileManager?.copyExistingDirectory(atPath: sourceProjectPath, toPath: destinationProjectPath)
        let destinationProjectLoadingInfo = ProjectLoadingInfo(forProjectWithName: uniqueDestinationProjectName, projectID: nil)
        let project = Project.init(loadingInfo: destinationProjectLoadingInfo!)
        project!.header.programName = destinationProjectLoadingInfo!.visibleName
        project!.saveToDisk(withNotification: true)
    }

    @objc static func defaultProject(projectName: String, projectID: String?) -> Project {
        let uniqueProjectName = Util.uniqueName(projectName, existingNames: ProjectService.getAllProjectNames())
        let project = Project()
        project.header = Header.default()
        project.header.programName = uniqueProjectName
        project.header.programID = projectID

        let fileManager = CBFileManager.shared()
        if !fileManager!.directoryExists(uniqueProjectName) {
